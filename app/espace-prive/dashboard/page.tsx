...
const Dashboard = async (props: Props) => {
  const [entities, totalPosts, activeModal, usersByRole, usersByGrade] = await Promise.all([
    db.entity.findMany({
      select: {
        id: true,
        name: true,
          ...
          <CardContent className="text-sm">
            {activeModal ? (
              <div>
                ...
                <div>
                  du {new Date(activeModal.startDate).toLocaleDateString('fr-FR')} au{' '}
                  {new Date(activeModal.endDate).toLocaleDateString('fr-FR')}
                </div>
                <Link
        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {entities.map((e) => {
              ...
              const usersForEntity = usersByRole.find((r) => String(r.role) === roleKey);
              ...
              return (
                <div key={e.id} className="flex items-center justify-between border rounded p-3">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-lg font-semibold">{e._count.blogpages}</div>
